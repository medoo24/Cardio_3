(() => {
  ...
  function markVisited(route) {
    const visited = new Set(JSON.parse(storage.get("valvelab-visited") || "[]"));
    visited.add(route);
    storage.set("valvelab-visited", JSON.stringify([...visited]));
    updateProgress();
  }
  ...
  function initQuiz() {
    ...
    let score = 0;
    let answered = false;
    const storedBest = Number(storage.get("valvelab-best-score") || 0);
    ...
    function renderResults() {
      ...
      storage.set("valvelab-best-score", String(best));
  ...
  document.getElementById("reset-progress").addEventListener("click", () => {
    storage.remove("valvelab-visited");
    updateProgress();
